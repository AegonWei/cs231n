...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    ##  Li = -np.log(P) = -np.log(np.exp(f[i]) / np.sum(np.exp(f))) = -f + np.log(np.exp(f))
        exp_sum = np.sum(np.exp(scores))
        loss += np.log(exp_sum) - correct_class_score
        
    ##  j = i, dW = (np.exp(scores) / np.sum(np.exp(scores)))*X[i] - X[i]
    ##  j != i, dW = (np.exp(scores) / np.sum(np.exp(scores)))*X[i]
        dW[:, y[i]] -= X[i]
